...
| timestamp_modified | An integer which toggles the display of <i>timestamp_modified</i> XML node. If this variable is set to 1 or 2 the timestamp_modified node is added to the XML and will contain the timestamp when the scan was most recently modified. Additionally, if <code>timestamp_modified=1</code>, then variables <b>start_date</b>, <b>start_time</b>, <b>end_date</b>, <b>end_time</b> will use the timestamp_modified value to perform filtering, instead of regular timestamp (which is the timestamp when the scan record was created). |
